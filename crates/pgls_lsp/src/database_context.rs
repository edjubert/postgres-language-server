...
pub(crate) struct DbshDatabaseContextRequest {
    pub(crate) context: Option<DbshDatabaseContext>,
}

pub(crate) struct DbshDatabaseContext {
    connection: DbshDatabaseConnection,
    search_path: Vec<String>,
}

struct DbshDatabaseConnection {
    host: String,
    port: u16,
    username: String,
    password: String,
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct DbshDatabaseContextWire {
    connection: DbshDatabaseConnection,
    search_path: Vec<String>,
}
...
#[derive(Deserialize)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
struct DbshDatabaseConnectionWire {
    host: String,
    port: u16,
    username: String,
    password: String,
...
impl<'de> Deserialize<'de> for DbshDatabaseContext {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        let wire = DbshDatabaseContextWire::deserialize(deserializer)?;
...
impl<'de> Deserialize<'de> for DbshDatabaseConnection {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: Deserializer<'de>,
    {
        let wire = DbshDatabaseConnectionWire::deserialize(deserializer)?;
...
impl fmt::Debug for DbshDatabaseContextRequest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DbshDatabaseContextRequest")
            .field("context", &self.context)
            .finish()
    }
}

impl fmt::Debug for DbshDatabaseContext {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DbshDatabaseContext")
...
impl fmt::Debug for DbshDatabaseConnection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("DbshDatabaseConnection")
...
#[cfg(test)]
mod tests {
    use super::DbshDatabaseContextRequest;
    use serde_json::json;
    ...
    fn validates_the_strict_database_context_payload() {
        for payload in [
            ...
            assert!(serde_json::from_value::<DbshDatabaseContextRequest>(payload).is_err());
        }
    }

    #[test]
    fn accepts_a_clear_context_and_redacts_password_from_debug() {
        let clear = serde_json::from_value::<DbshDatabaseContextRequest>(json!({
            "context": null
        }))
        .expect("clear context must deserialize");
        assert!(clear.context.is_none());

        let context = serde_json::from_value::<DbshDatabaseContextRequest>(json!({
            "context": {
                "connection": {
                    "host": "localhost",
                    "port": 5432,
