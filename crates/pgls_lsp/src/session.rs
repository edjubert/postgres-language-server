...
use crate::database_context::DbshDatabaseContext;
...
pub(crate) struct Session {
    ...
    /// Session-only database context supplied by dbsh's managed client.
    dbsh_database_context: RwLock<Option<PartialConfiguration>>,
...
impl Session {
    pub(crate) fn new(
        ...
        Self {
            ...
            env_config,
            dbsh_database_context: RwLock::default(),
    ...
    pub(crate) fn set_dbsh_database_context(&self, context: Option<DbshDatabaseContext>) {
        *self.dbsh_database_context.write().unwrap() =
            context.map(DbshDatabaseContext::into_partial_configuration);
    }
    ...
    async fn load_pgls_configuration_file(
        ...
        match load_configuration(&self.fs, base_path.clone()) {
            Ok(loaded_configuration) => {
                ...
                let dbsh_database_context = self.dbsh_database_context.read().unwrap().clone();
                fs_configuration = merge_configuration_layers(
                    fs_configuration,
                    extra_config,
                    self.env_config.as_ref(),
                    dbsh_database_context.as_ref(),
                );
                let fs = &self.fs;
...
fn merge_configuration_layers(
    ...
    dbsh_database_context: Option<&PartialConfiguration>,
    ...
    if let Some(dbsh_database_context) = dbsh_database_context {
        let dbsh_search_path = dbsh_database_context
            .typecheck
            .as_ref()
            .and_then(|typecheck| typecheck.search_path.clone());
        filesystem_configuration.merge_with(dbsh_database_context.clone());
        if let Some(database) = filesystem_configuration.db.as_mut() {
            database.connection_string = None;
        }
        if let Some(search_path) = dbsh_search_path {
...
mod tests {
    ...
    #[test]
    fn dbsh_context_overrides_project_and_environment_configuration() {
        ...
        let dbsh_context = PartialConfiguration {
            db: Some(PartialDatabaseConfiguration {
                host: Some("dbsh-host".to_string()),
        ...
        let merged =
            merge_configuration_layers(project, None, Some(&environment), Some(&dbsh_context));
