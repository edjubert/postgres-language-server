...
#[sqlx::test(migrator = "pgls_test_utils::MIGRATIONS")]
async fn dbsh_database_context_sets_database_and_ordered_search_path(
    test_db: PgPool,
) -> Result<()> {
    let factory = ServerFactory::default();
    let mut fs = MemoryFileSystem::default();
    ...
    test_db
        .execute(
            r#"
                create schema dbsh_first;
                create schema dbsh_second;
                create table dbsh_first.context_users (id integer);
            "#,
        )
        .await
        .expect("failed to set up dbsh context schemas");
        ...
        .request::<_, ()>(
            ...
            json!({
                "context": {
                    ...
                    "searchPath": ["dbsh_first", "dbsh_second", "public"]
                }
            }),
        )
        .await?
    ...
    assert!(
        server
            .request::<_, ()>(
                "pgls/setDatabaseContext",
                "_reject_invalid_dbsh_database_context",
                json!({
                    "context": {
                        "connection": {
                            "host": opts.get_host(),
    ...
    assert!(
        ...
        "expected a database-backed diagnostic through the dbsh context"
    );
